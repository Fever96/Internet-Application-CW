...
int sock;                           //
struct sockaddr_in sendAddr;		//发端 Add
struct sockaddr_in serverAddr;			//Server ADD
struct sockaddr_in selfAddr; 		//Self ADD

char sendBuf[MAX_SIZE];				//发送存在内存里
char recvBuf[MAX_SIZE];				//接受也存在内存里

int sendSize = 312;
int recvSize;

int svrAddrLen;						//Server Addr Len 
int counter = 0;					//填充计数器
int discoverFlag = 0;				// 用于 discovery 的一个标记
int broadcastFlag = 0;				// 用于广播的标记

unsigned int DHCPServerIPAddress = 0xffffffff;	//广播时的Server地址
...
void recvMESSAGE(int type, int subType);
void sendMESSAGE(int type, int subType);
unsigned int getTID();
void getMAC(unsigned char MAC[6]);
unsigned int getIP();
int SetClientAddress(char *ifname, char *Ipaddr, char *mask,char *gateway);
...
int main(int argc, char *argv[])
{
    ...
    setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));       //设置套接口

    int x = 1;
    setsockopt(sock, SOL_SOCKET, SO_BROADCAST | SO_REUSEADDR, &x, sizeof(int));

    memset(&selfAddr, 0, sizeof(selfAddr));     //重置内存
    ...
    // DHCP Packet
    unsigned char bootRequest = (0x01);             //定义请求
    memcpy(&sendBuf[0], &bootRequest, sizeof(unsigned char));       //拷贝内存

    unsigned char hardwareType = (0x01);            //定义硬件类型
    memcpy(&sendBuf[1], &hardwareType, sizeof(unsigned char));

    unsigned char hardwareAddressLength = (0x06);       //定义硬件长度
    memcpy(&sendBuf[2], &hardwareAddressLength, sizeof(unsigned char));

    unsigned char hops = (0x00);                //定义跳变 同一网内为0
    memcpy(&sendBuf[3], &hops, sizeof(unsigned char));

    unsigned int XID = htonl(getTID());
    ...
    if(argc == 2)
    {
        if(strcmp("--discover", argv[1]) == 0) // Discover, Offer, Request, ACK
        {
            type = 1;
            subType = 1;
            sendMESSAGE(type, subType);
        }
        else if(strcmp("--renew1-U", argv[1]) == 0) // T1 Expire #!!!!Time out
        {
            type = 3;
            subType = 2;
            sendMESSAGE(type, subType);
        }
        else if(strcmp("--renew2-B", argv[1]) == 0) // T2 Expire  #!!!!Time out
        {
            type = 3;
            subType = 1;
            sendMESSAGE(type, subType);
        ...
        else if(strcmp("--init", argv[1]) == 0)
        {
            ...
            SetClientAddress("eth1", IPPointer, MASKPointer, RouterPointer);
        ...
        else if(strcmp("--interact", argv[1]) == 0)
        {
            ...
            // start the thread
            thd = pthread_create(&thd1, NULL, thread1, &thd1);  
            ...
            while(1)
            {
                ...
                printf(">");
                scanf("%s", s1);
               	//printf("tmd weishenme bu tingzhu!!!!!\n");					//不等待输入
...
void recvMESSAGE(int type, int subType)				//接受 MSG
{
	
    switch(type){
        case 1:{ // Read the DHCP Offer, send DHCP Request
            svrAddrLen = sizeof(serverAddr);
            recvSize = recvfrom(sock, recvBuf, MAX_SIZE, 0,(struct sockaddr *) &serverAddr, &svrAddrLen);
            ...
            else
            {
                ...
                sendMESSAGE(3, 1); // For Discover-Offer-Request-ACK loop
            }
            break;
        }
        ...
        case 3:{ // Read the DHCP ACK, send nothing
            ...
            else if (MessageType == 0x06) // NAK
            {
                ...
                sendMESSAGE(1, 1); // For Discover-Offer-Request-ACK loop
            }
            else
            	;
            break;
        ...
        case 8:{ // Read the DHCP ACK, send nothing
            svrAddrLen = sizeof(serverAddr);
            if ((recvSize = recvfrom(sock, recvBuf, MAX_SIZE, 0,(struct sockaddr *) &serverAddr, &svrAddrLen)) < 312)
                printf("recvfrom() failed.\n");
...
{	
	...
	//读取 DHCP 配置文件 将里面的 ip 写进 DHCPServerIPAddress
    FILE* DHCPFile;
    ...
    switch(type)
    {
        ...
        case 3: // DHCP Requestuest
        {
            if(subType == 1) // For Discover-Offer-Request-ACK loop
            {
                if(discoverFlag == 1)
                {
    ...
    if(broadcastFlag == 1)				//广播时地址为255.255.255.0
        sendAddr.sin_addr.s_addr = 0xffffffff;/*Server IP address*/
    else
        sendAddr.sin_addr.s_addr = DHCPServerIPAddress;			//单播时地址为文件中地址 
    sendAddr.sin_port = htons(67); /* Server port */

    if(type == 1 || type == 3 || type == 7 || type == 8)
    {
        if(type == 7 && getIP() == 0x01010101)				//此处未写好	
            ;
        else
            length = sendto(sock, sendBuf, sendSize, 0, (struct sockaddr *) &sendAddr, sizeof(sendAddr));		//发送包
    }

    if(type == 1 || type == 3 || type == 8)
    {
        recvMESSAGE(type, subType);
        discoverFlag = 0;
    } 
    else if(type == 7)
    {
...
int SetClientAddress(char *ifname, char *Ipaddr, char *mask, char *gateway)
{
    ...
    {     
        perror("ioctl SIOCSIFADDR(IP Address) error");     
        return -3;     
    }
...
{  
    ...
    while(1)
    {
        ...
        t = time(NULL);  				//获取系统时间
        //struct tm *lt;  
        int timestamp = time(&t);		//此处 timestamp 为系统时间戳
        if(timestamp > timer)
        {
            timer = timestamp;
            i++;
