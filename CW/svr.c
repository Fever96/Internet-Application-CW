        ...
        switch(recvBuf[242])
        {
            case 0x01: // DHCP Discover
            {
                ...
                else
                {
                    ...
                    printf("0x02:%d\n",sendBuf[0]);
                    memcpy(&sendBuf[0], &messageType, sizeof(messageType));
					printf("0x02:%d\n",sendBuf[0]);
                    ...
                    memcpy(&sendBuf[242], &option53_DhcpMessageType, sizeof(option53_DhcpMessageType));

                    // Option 54
                    unsigned char option54 = (0x36);
                    memcpy(&sendBuf[243], &option54, sizeof(option54));

                    unsigned char option54_Length = (0x04);
                    memcpy(&sendBuf[244], &option54_Length, sizeof(option54_Length));

                    unsigned int option54_DhcpServerIdentifier = getIP();
                    memcpy(&sendBuf[245], &option54_DhcpServerIdentifier, sizeof(option54_DhcpServerIdentifier));
                    printf("option54:%d",option54_DhcpServerIdentifier);

                    // Option 51
                    unsigned char option51 = (0x33);
                    memcpy(&sendBuf[249], &option51, sizeof(option51));

                    unsigned char option51_Length = (0x04);
                    memcpy(&sendBuf[250], &option51_Length, sizeof(option51_Length));

                    unsigned int option51_IPAddressLeaseTime = LEASETIME;
                    memcpy(&sendBuf[251], &option51_IPAddressLeaseTime, sizeof(option51_IPAddressLeaseTime));

                    // Option 1
                    unsigned char option1 = (0x01);
                    memcpy(&sendBuf[255], &option1, sizeof(option1));

                    unsigned char option1_Length = (0x04);
                    memcpy(&sendBuf[256], &option1_Length, sizeof(option1_Length));

                    unsigned int option1_SubnetMask = MASK;
                    memcpy(&sendBuf[257], &option1_SubnetMask, sizeof(option1_SubnetMask));

                    // Option 3
                    unsigned char option3 = (0x03);
                    memcpy(&sendBuf[261], &option3, sizeof(option3));

                    unsigned char option3_Length = (0x04);
                    memcpy(&sendBuf[262], &option3_Length, sizeof(option3_Length));

                    unsigned int option3_Router = ROUTER;
                    memcpy(&sendBuf[263], &option3_Router, sizeof(option3_Router));

                    // Option 6
                    unsigned char option6 = (0x06);
                    memcpy(&sendBuf[267], &option6, sizeof(option6));

                    unsigned char option6_Length = (0x04);
                    memcpy(&sendBuf[268], &option6_Length, sizeof(option6_Length));

                    unsigned int option6_DNS = DNS;
                    memcpy(&sendBuf[269], &option6_DNS, sizeof(option6_DNS));
...
// Change the IP address stored in the configuration file
void offerIP(unsigned char MAC[6])
{
    FILE *infile;
    FILE *outfile;
...
// Retrive the IP when received release
void retrieveIP(unsigned int IP)
{
    FILE *infile;
    FILE *outfile;
